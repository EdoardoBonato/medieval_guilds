...
mocarelli['Name of the guild'] = mocarelli['Name of the guild'].str.upper()
senato_path = r"C:\Users\edobo\OneDrive\Desktop\Thesis\Medieval Guilds\Data\EDITED_db\Merge\senato_integration.xlsx"
senato = pd.read_excel(senato_path)

#see common guilds
common_guilds = pd.merge(mocarelli, senato, on = ['Place', 'Name of the guild'], how = 'inner')
common_guilds = common_guilds[['Place', 'Name of the guild']]

#merging operation
merged = pd.merge(mocarelli,senato, how= 'outer')
merged['Name of the guild'] = merged['Name of the guild'].str.upper()
...
duplicate_rows = merged[merged.duplicated(subset=['Place', 'Name of the guild'])]

#different merging technique
merged_try = pd.merge(mocarelli, senato, on = ['Place', 'Name of the guild'], how = 'outer')
duplicate_rows_try = merged_try[merged_try.duplicated(subset=['Place', 'Name of the guild'])]
...
#try to group
grouped = merged_try.groupby(['Name of the guild', 'Place'])
partial_merge = {}
...
for name, group in grouped:
    ...
    concatenated_years = concatenated_years[(concatenated_years >= 800) & (concatenated_years <= 1800)].sort_values()
    # Create a DataFrame for the current group with the concatenated years and the name and place
    current_group_df = pd.DataFrame({'Name of the guild': name[0], 'Place': name[1], 'years': [concatenated_years.values]})
    # Append the current group's DataFrame to the concatenated results DataFrame
    total_merge[name] = current_group_df
    total_merge_df = pd.concat([total_merge_df, current_group_df], ignore_index=True)

total_merged = pd.merge(mocarelli, total_merge_df, on = ['Place', 'Name of the guild'], how = 'outer')
...
total_merged['check_discrepancies'] = [np.nan if np.isnan(value['Year']) or np.isnan(value['year_']) else( 1 if value['Year'] - 20 < value['year_'] < value['Year'] + 20 else 0) for index, value in total_merged.iterrows()]
...
total_merged.to_excel(r'C:\Users\edobo\OneDrive\Desktop\Thesis\Medieval Guilds\Data\EDITED_db\Merge\merge_senato_mocarelli.xlsx')
merged_try.to_excel(r'C:\Users\edobo\OneDrive\Desktop\Thesis\Medieval Guilds\Data\EDITED_db\Merge\merge_senato_mocarelli_try.xlsx')