...
"""
"""
This script is used to scrape the data from the website of the Italian Senate where the Chelazzi catalogue is uploaded, in more than 800  pdfs.
The data are organized in 800 different pages, one for each document. In order to create the excel, output of this script two tasks are required.
    1. The information about Place and Name of the Guilds (which is may be difficultly retrievable from the PDF itself) is extracted and stored.
    2. The pdf itself is downloaded.
The data is then saved in an Excel file.   
One may think about dynamic webscraping, but in this case a static approach is used, replicating the java function that is used to redirect to the link downloading the pdf
"""
...
# replicate the java function which is used to redirect to the pdfs and download them #
n = 0
gilde = []    
for url in URL:
...
gilde.to_excel(r"check_senato_output.xlsx")
